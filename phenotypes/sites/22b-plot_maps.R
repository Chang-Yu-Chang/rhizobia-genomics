...
p1 <- map_states %>%
    ...
    # MLBS
    annotate("rect", color = population_colors["VA"], fill = NA, linewidth = 1,
    ...
    # Phila
    annotate("rect", color = population_colors["PA"], fill = NA, linewidth = 1,
...
p2 <- ggmap(map_phila) +
    ...
    theme_void() +
    theme(
        plot.title = element_text(margin = margin(t = 10, b = -20), hjust = 0.1, color = population_colors["PA"]),
        panel.border = element_rect(color = population_colors["PA"], fill = NA, linewidth = 2),
        legend.position = c(.5,.5),
        legend.key.size = unit(5, "mm"),
        legend.background = element_rect(color = 1, fill = "white")
    ) +
    guides() +
    labs(x = "longitude", y = "latitude", title = "PA")
...
p3 <- ggmap(map_mlbs) +
    ...
    theme_void() +
    theme(
        plot.title = element_text(margin = margin(t = 10, b = -20), hjust = 0.1, color = population_colors["VA"]),
        panel.border = element_rect(color = population_colors["VA"], fill = NA, linewidth = 2),
        legend.position = "none",
        legend.key.size = unit(5, "mm"), # = element_text(size = 5),
        legend.background = element_rect(color = 1, fill = "white")
    ) +
    guides() +
    labs(x = "longitude", y = "latitude", title = "VA")
