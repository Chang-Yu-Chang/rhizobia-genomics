...
plants_r <- plants %>%
    ...
    summarize(across(-1,function (x) mean(x, na.rm = T))) %>%
...
for (i in 1:3) {
    ...
    png(paste0(folder_data, "phenotypes/tradeoff/0", i, ".png"), width = 20, height = 20, units = "cm", res = 1000)
    ...
    corrplot(
        ...
        type = "full", tl.col = "black", tl.cex = 1, cl.ratio = 0.1, tl.srt = 45, tl.offset = .1
    ) %>%
