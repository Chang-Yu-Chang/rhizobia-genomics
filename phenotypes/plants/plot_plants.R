...
# 3. Reaction norm for N supplement ----
plants_n <- plants %>%
...
plants_n_summ <- plants_n %>%
    ...
    summarize(trait_mean = mean(value))

p <- plants_n %>%
    ggplot(aes(x = exp_nitrogen, y = value, color = population, fill = population)) +
    geom_dotplot(binaxis = 'y', stackdir = 'center', dotsize = .1) +
    geom_violin(position = "identity", alpha = 0.5, color = NA, trim = T) +
    geom_line(data = plants_n_summ, aes(x = exp_nitrogen, y = trait_mean, group = population), linewidth = 1) +
    geom_point(data = plants_n_summ, aes(x = exp_nitrogen, y = trait_mean), position = "identity", size = 2, shape = 21, stroke = 1, fill = "white") +
    #geom_boxplot(outlier.shape = -1, alpha = 0.5) +
    #geom_point(size = 1, shape = 21, position = position_dodge(width = .5)) +
    #geom_jitter(size = .1, shape = 21, width = .2) +
    scale_color_manual(values = population_colors) +
    scale_fill_manual(values = population_colors) +
    #facet_wrap2(~trait_pre, nrow = 2, scales = "free_y") +
    facet_nested(
...
p
ggsave(paste0(folder_phenotypes, "plants/03-sativa_nitrogen.png"), p, width = 16, height = 5)
