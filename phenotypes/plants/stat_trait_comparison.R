...
#' It outputs four tables for each plant
...
write_csv(tb_tidied, paste0(folder_data, "phenotypes/plants/sativa/pairs_anova.csv"))
...
write_csv(tb_tidied3, paste0(folder_data, "phenotypes/plants/sativa/pairs_perm.csv"))
...
write_csv(pairs_perm_obv, paste0(folder_data, "phenotypes/plants/sativa/pairs_perm_obv.csv"))
write_csv(pairs_perm_raw, paste0(folder_data, "phenotypes/plants/sativa/pairs_perm_raw.csv"))
...
# 2.0 Prepare data ----
plants_n <- plants %>%
    filter(population != "control", exp_plant == "lupulina", exp_nitrogen == "N-") %>%
    select(gradient, population, site, exp_id, genome_id, genome_name, exp_plant, exp_plantsite, exp_plantmaternal, exp_waterblock, shoot_biomass_mg, nodule_number, root_biomass_mg) %>%
    group_by(gradient, population, exp_plant) %>%
    pivot_longer(cols = -c(1:10), names_to = "trait", values_drop_na = T) %>%
    left_join(traits) %>%
    left_join(isolates) %>%
    ungroup()
...
tb <- tibble(
    gradient = c(rep("elevation", 3), rep("urbanization", 3)),
    trait_pre = c(
        # Elevation traits
        "shoot\nbiomass (mg)",
        "nodule number",
    ...
    st = c(
        # Elevation traits
        "mod <- lmer(value ~ population + (1|exp_id) + (1|exp_plantmaternal) + (1|exp_waterblock), data = d)",
        "mod <- glmer(value ~ population + (1|exp_id) + (1|exp_plantmaternal) + (1|exp_waterblock), family = 'poisson', data = d)",
        "mod <- lmer(value ~ population + (1|exp_id) + (1|exp_plantmaternal) + (1|exp_waterblock), data = d)",
        # Urbanization traits
        "mod <- lmer(value ~ population + (1|exp_id) + (1|exp_waterblock), data = d)",
        "mod <- glmer(value ~ population + (1|exp_id) + (1|exp_waterblock), family = 'poisson', data = d)",
        "mod <- lmer(value ~ population + (1|exp_id) + (1|exp_waterblock), data = d)"
    )
) %>%
    mutate(
...
write_csv(tb_tidied, paste0(folder_data, "phenotypes/plants/lupulina/pairs_anova.csv"))
...
write_csv(tb_tidied3, paste0(folder_data, "phenotypes/plants/lupulina/pairs_perm.csv"))
...
pairs_perm_obv <- tb_tidied2 %>%
    filter(!str_detect(term, "Intercept")) %>%
    select(gradient, trait_pre, trait, term, statistic)
write_csv(pairs_perm_obv, paste0(folder_data, "phenotypes/plants/lupulina/pairs_perm_obv.csv"))
write_csv(pairs_perm_raw, paste0(folder_data, "phenotypes/plants/lupulina/pairs_perm_raw.csv"))
