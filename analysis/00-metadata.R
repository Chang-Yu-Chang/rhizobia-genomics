...
genomes_mapping <- tibble(
    batch_name = c(rep("Chang_Q5C_results", 10), "Chang_Q5C_results_repeated", rep("Chang_Q5C_results", 6), "Chang_Q5C_results_repeated", "Chang_Q5C_results",
                    ...
                    rep("Buxton-Martin_G2N_results", 6),
                    rep("ncbi", 4)),
    sample_id = c(paste0("Chang_Q5C_", 1:19), paste0("Chang_W8S_", 1:18), paste0("Buxton-Martin_G2N_", 1:6),"usda1106", "em1021", "em1022", "wsm419"),
    accession = c(rep(NA, 43), c("GCF_002197065.1", "GCF_000006965.1", "GCF_013315775.1", "GCF_000017145.1")),
    genome_name = c(paste0("Chang_Q5C_", 1:19), paste0("Chang_W8S_", 1:18), paste0("Buxton-Martin_G2N_", 1:6), "usda1106", "em1021", "em1022", "wsm419"),
    genome_id = factor(c(paste0("g", 1:43), "usda1106", "em1021", "em1022", "wsm419")),
)
...
# Genomes that need to be assembled locally
genomes <- isolates %>%
    drop_na(exp_id) %>%
    filter(str_detect(genome_id, "g")) %>%
    left_join(select(genomes_mapping, batch_name, genome_name)) %>%
    select(batch_name, genome_name, genome_id, exp_id)
write_csv(genomes, paste0(folder_data, "temp/00-genomes.csv"))
