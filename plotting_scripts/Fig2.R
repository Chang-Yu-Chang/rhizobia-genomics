...
bar_y <- 1.3
plot_dots <- function (igcl, igcm, sigs, pop) {
    igcl <- filter(igcl, population == pop)
    igcm <- filter(igcm, population == pop)
    sigs <- filter(sigs, population == pop)
    ...
    igcl %>%
        ...
        facet_wrap(population~., scales = "free_x", nrow = 2, labeller = labeller(population = c(VA = "mountain", PA = "city"))) +
        theme_classic() +
        theme(
...
p1_1 <- plot_dots(igcl, igcm, sigs, "VA")
p1_2 <- plot_dots(igcl, igcm, sigs, "PA")


# Panel B. PCoA plot of the three growth traits at 30C ----
...
##
p2_1 <- bind_cols(isolates_i1, pca1$x[,1:2]) %>% plot_pca(eigs1[1], eigs1[2])
...
plot_dots2 <- function (pl, plm, sigs, pop) {
    pl <- filter(pl, population == pop)
    plm <- filter(plm, population == pop)
    sigs <- filter(sigs, population == pop)
...
p3_1 <- plot_dots2(pl, plm, sigs, "VA")
p3_2 <- plot_dots2(pl, plm, sigs, "PA")
