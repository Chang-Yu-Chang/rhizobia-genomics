...
p1 <- dml %>%
    ...
    scale_color_manual(values = c(Virginia = "steelblue", Pennsylvania = "#db7272")) +
...
p2 <- dml %>%
    ...
    geom_line(aes(x = yday, y = tmax_deg_c, group = site, color = population, linetype = "T max")) +
    geom_line(aes(x = yday, y = tmin_deg_c, group = site, color = population, linetype = "T min"), alpha = .5) +
    scale_x_continuous(expand = c(0, 0), breaks = 1:365) +
...
# ----
p <- plot_grid(
    p1, plot_grid(p2, p3, p4, nrow = 1, rel_widths = c(2,1,1), align = "h", axis = "tb", labels = LETTERS[2:4]),
