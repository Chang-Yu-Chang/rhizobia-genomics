...
library(ggcorrplot) # For comparing correlation
...
for (i in 1:3) {
    ...
    p_mat <- cor_pmat(m, conf.level = 0.95)
    tt <- min(which(str_detect(names(m), "^r_"))) # the index of first growth trait; for plotting

    p_traits[[i]] <- ggcorrplot(mm, method = "circle",  hc.order = TRUE, outline.col = "white", type = "full", p.mat = p_mat) +
        scale_fill_gradientn(colours = c("blue", "white", "red"), values = scales::rescale(c(1, .5, 0, -.5, -1)), name = "") +
        ...
        guides() +
        labs() +
        ggtitle(paste0(list_treatments[i], ", n_strains = ", nrow(isolates_trait)))
...
#  ----
p <- plot_grid(
    p_traits[[2]], p_traits[[1]], p_pcs[[2]], p_pcs[[1]],
    labels = LETTERS[1:4], scale = c(1,1,.9, .9)
) +
    theme(plot.background = element_rect(color = NA, fill = "white"))
ggsave(here::here("plots/FigS3.png"), p, width = 10, height = 10)
