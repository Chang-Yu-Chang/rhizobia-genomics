...
p1 <- tt$gpatl %>%
    ...
    scale_y_continuous(breaks = c(0:10), limits = c(0, 5)) +
...
c(singleton = gg[[1]], core = last(gg), single_core = nrow(tt$list_sccg), total = sum(gg)) # 2020         3741         2929       16135
last(gg)/sum(gg) *100 #
...
ggsave(here::here("plots/FigS2.png"), p, width = 6, height = 6)
