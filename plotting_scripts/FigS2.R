...
plot_box <- function(diff_var_i) {
    diff_var_i %>%
        ...
        scale_x_discrete(breaks = c("tmax_deg_c", "tmin_deg_c"), labels = c(expression(t[a]), expression(t[i]))) +
        theme_classic() +
        theme(
...
p1 <- plot_box(diff_var1) +
    ...
    ylab(expression(mean ~ "[" ~ t ~ "("~L~")" - t~ "("~H~")" ~ "]"(degree*C))) +
    ggtitle("Elevation")
p2 <- plot_box(diff_var2) +
    scale_y_continuous(limits = c(-2, 7), breaks = -2:7, expand = c(0,.1)) +
    ylab(expression(mean ~ "[" ~ t ~ "("~U~")" - t~ "("~S~")" ~ "]"(degree*C))) +
