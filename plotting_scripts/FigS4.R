...
tt <- read_gpas()
gpatls <- tt$gpatl %>% left_join(select(ani, genome_id, organism_name))
...
# meliloti
p1_1 <- gpatls %>%
    filter(organism_name == "Sinorhizobium meliloti") %>%
    compute_gfs() %>%
    plot_gfs() +
    theme(plot.title = element_text(face = "italic", size = 10)) +
    labs(title = "Sinorhizobium meliloti")
...
# medicae
p1_2 <- gpatls %>%
    filter(organism_name == "Sinorhizobium medicae") %>%
    compute_gfs() %>%
    plot_gfs() +
    theme(plot.title = element_text(face = "italic", size = 10)) +
    labs(title = "Sinorhizobium medicae")
...
tb1 <- tt$gpa %>%
    select(gene, matches(paste0(ani$genome_id[ani$organism_name == "Sinorhizobium meliloti"], "$"))) %>%
    do_sampling(n_boots = 100)
p2_1 <- plot_sampling(tb1$tbp, tb1$tbpr)

tb2 <- tt$gpa %>%
    select(gene, matches(paste0(ani$genome_id[ani$organism_name == "Sinorhizobium medicae"], "$"))) %>%
    do_sampling(n_boots = 100)
