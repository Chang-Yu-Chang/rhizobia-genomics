...
p1 <- diff_tmax_summer2 %>%
    ...
    geom_vline(xintercept = mean(diff_tmax_summer2$mean_diff_tmax), color = "maroon", linetype = 2, linewidth = 1) +
...
p2 <- diff_tmin_summer2 %>%
    ...
    geom_vline(xintercept = 0, color = "black", linetype = 2, linewidth = 1) +
    geom_vline(xintercept = mean(diff_tmin_summer2$mean_diff_tmin), color = "maroon", linetype = 2, linewidth = 1) +
    scale_x_continuous(expand = c(0,0), limits = c(-3, 8), breaks = seq(-5, 10, 5)) +
    theme_classic() +
    theme(
        panel.border = element_rect(color = "black", fill = NA)
...
p3 <- diff_tmax_jul2 %>%
    ...
    geom_histogram(aes(x = mean_diff_tmax), color = "black", fill = "white", binwidth = 0.5) +
    geom_vline(xintercept = 0, color = "black", linetype = 2, linewidth = 1) +
    geom_vline(xintercept = mean(diff_tmax_jul2$mean_diff_tmax), color = "maroon", linetype = 2, linewidth = 1) +
    scale_x_continuous(expand = c(0,0), limits = c(-3, 8), breaks = seq(-5, 10, 5)) +
    theme_classic() +
    theme(
        panel.border = element_rect(color = "black", fill = NA)
...
p4 <- diff_tmin_jul2 %>%
    ...
    geom_histogram(aes(x = mean_diff_tmin), color = "black", fill = "white", binwidth = 0.5) +
    geom_vline(xintercept = 0, color = "black", linetype = 2, linewidth = 1) +
    geom_vline(xintercept = mean(diff_tmin_jul2$mean_diff_tmin), color = "maroon", linetype = 2, linewidth = 1) +
    scale_x_continuous(expand = c(0,0), limits = c(-3, 8), breaks = seq(-5, 10, 5)) +
    theme_classic() +
    theme(
        panel.border = element_rect(color = "black", fill = NA)
...
# Stat
mean(diff_tmax_summer2$mean_diff_tmax) # 3.371116
t.test(diff_tmax_summer2$mean_diff_tmax)

mean(diff_tmin_summer2$mean_diff_tmin) # 0.5318582
t.test(diff_tmin_summer2$mean_diff_tmin)

mean(diff_tmax_jul2$mean_diff_tmax) # 3.211613
t.test(diff_tmax_jul2$mean_diff_tmax)

mean(diff_tmin_jul2$mean_diff_tmin) # 0.8837774
