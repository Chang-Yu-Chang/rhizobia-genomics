    ...
    for x in np.nditer(sequenceMatrix, order='F',flags=['external_loop']): 
        ...
        # Monomorphic sites. Try to clean out of the loop
            ...
            if((np.unique(pol).shape[0] == 1) and (np.unique(pol)[0] != AA)):
                ...
                output.append(tmp)
    return(output)
def formatSfs(sequenceMatrix,rawSfsOutput,dafFile,divFile,path,append=False):
    ...
    sequence_array = np.array(sequenceMatrix[0])
    div['mi'] = np.sum(sequence_array == '0')
    div['m0'] = np.sum(sequence_array == '4')
    ...
    P0_values = daf[daf['functionalClass'] == '4fold']['derivedAlleleFrequency'] if '4fold' in daf['functionalClass'].values else pd.Series(dtype=float)
    Pi_values = daf[daf['functionalClass'] == '0fold']['derivedAlleleFrequency'] if '0fold' in daf['functionalClass'].values else pd.Series(dtype=float)
    
    sfs = pd.DataFrame({
        'daf': daf['categories'].unique(),
        'P0': P0_values.reset_index(drop=True),
        'Pi': Pi_values.reset_index(drop=True)
    })
...
# Open multi-Fasta
file = px.Fasta(multiFasta,duplicate_action='first',sequence_always_upper=True,read_long_names=True)
...
formatSfs(multiFastaMatrix,rawSfs,dafFile,divFile,path)
