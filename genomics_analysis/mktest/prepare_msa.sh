...
set_name="elev_med"
ref="em1021"
blast_db=$folder_data/genomics_analysis/mktest/outgroup/$ref/$ref

mamba activate blast
blastn \
    -query $query_fasta \
    -db $blast_db \
    -out $folder_data/genomics_analysis/mktest/$set_name/blast_results.txt \
...
folder_refseq=$folder_data/genomics_analysis/mktest/$set_name/refseq # temp folder for extracted sequence from refernece
blast_results=$folder_data/genomics_analysis/mktest/$set_name/blast_results.txt # blast table
ref_fasta=$folder_data/genomics_analysis/mktest/outgroup/$ref/$ref.fasta # reference genome
...
while IFS=$'\n' read -r line; do
    ...
    query_name=$(echo "$line" | awk -F'\t' '{print $1}')
    ref_id=$(echo "$line" | awk -F'\t' '{print $2}')

    # Extract the matched reference sequence using seqkit or other tools
    mamba activate seqkit
    ref_seq=$(seqkit grep -n -p $ref_id $ref_fasta)  # Adjust based on how ref_id is formatted
    ...
    msa_file=$folder_sourcemsa/${query_name}.aln.fas  # Assuming MSAs are named after query names
