...
compute_by_gradient <- function (gidi) {
    #' Compute the fst by gradient. gidi is subset from gid
    #gidi <- gid1
    n_alleles <- nAll(gidi)
    ...
    dstat <- diff_stats(gidi)
    tb_snp_locus <- bind_cols(tibble(snp_id = rownames(dstat$per.locus)), as_tibble(dstat$per.locus))
    tb_snp_global <- dstat$global
    # dtest_snp <- diff_test(gidi)
    # tb_snp <- bind_cols(tibble(snp_id = rownames(dstat$per.locus), fisher_p = dtest_snp), as_tibble(dstat$per.locus))
...
# Set 2. core genes shared by 33 symbiotic strains
