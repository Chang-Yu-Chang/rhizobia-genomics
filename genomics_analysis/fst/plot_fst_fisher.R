...
ggsave(paste0(folder_data, "genomics_analysis/fst/04-set1_elevation.png"), p, width = 10, height = 10)


# 5. Fst manhattan plot in urban----
gene_order <- read_csv(paste0(folder_data, "genomics_analysis/gene_content/gene_order.csv")) %>%
    mutate(gene_id = factor(1:n()))
gpa_fis <- read_csv(paste0(folder_data, "genomics_analysis/fst/gpa_fis.csv"))
grt <- gpacl %>%
...
p1 <- gpacl %>%
    ...
    ggplot() +
    geom_tile(aes(x = gene_id, y = genome_id, fill = site_group)) +
    scale_fill_manual(values = alpha(site_group_colors[3:4], 0.8))+
    scale_y_discrete(expand = c(0,0), position = "right") +
    facet_grid(site_group ~ replicon_type, scales = "free", space = "free", switch = "y") +
    coord_cartesian(clip = "off") +
    theme_classic() +
...
ggsave(paste0(folder_data, "genomics_analysis/fst/05-set1_urbanization.png"), p, width = 10, height = 10)
