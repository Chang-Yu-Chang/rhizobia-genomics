...
join_dists <- function (ind_dxy, gene_length, gene_wide_fst, gpacl, sites_d, isolates) {
    ...
    ind_dxy %>%
        ...
        left_join(select(gene_wide_fst, gene, n_snps = fst.n_snps)) %>%
...
plot_genome_wide_dxy <- function (xx_gn) {
    ...
    xx_gn %>%
        ...
        annotate("text", x = -Inf, y = Inf, label = paste(xx_snps$n_snps, "SNPs", ", R² =", round(r_squared, 3), p_value), hjust = -.2, vjust = 1.5, size = 3, color = "black") +
        theme_bw() +
        theme(
...
plot_replicon_wide_dxy <- function (xx_rp) {
    ...
    xx_rp %>%
        ...
        geom_smooth(aes(x = dist_geo_km, y = dxy_scaled), method = "lm", se = F, color = "black") +
        geom_point(aes(x = dist_geo_km, y = dxy_scaled, color = pops), shape = 21, size = 2, stroke = 1) +
        geom_text(data = lm_results, aes(label = paste(n_snps, "SNPs, R² =", round(r_squared, 3), ast)), x = -Inf, y = Inf, hjust = -.1, vjust = 1.5, size = 3, color = "black") +
        scale_color_manual(values = pops_colors) +
...
xx <- join_dists(dd$gene_ind_dxy, dd$gene_lengths, dd$gene_wide_fst, tt$gpacl, sites_dist, isolates) # Per gene, dxy between two orthologs coming from two different genomes
