...
curation_wrapper <- function () {
    tbtr <- tibble(tr_type = c("core", "gpa", "ani", "kmer"), tr = NA)
    ...
    tbtr$tr[3] <- list(read.tree(paste0(folder_data, "phylogenomics_analysis/trees/tr_ani.tree")))
    tbtr$tr[4] <- list(read.tree(paste0(folder_data, "phylogenomics_analysis/trees/tr_kmer.tree")))
    return(tbtr)
}
