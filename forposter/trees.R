...
p1 <- tr_seq_core %>%
    ...
    geom_nodepoint(aes(label = highlight_boot, color = "bootstrap>95%"), shape = 16, size = 3, alpha = 0.3) +
    geom_tiplab(hjust = -.8, size = 3) +
    ...
    scale_color_manual(values = c(site_group_colors, `bootstrap>95%`="grey50"), name = NULL, breaks = c("high elevation", "low elevation", "bootstrap>95%")) +
    scale_linetype_manual(values = c(1,5), name = NULL) +
    coord_cartesian(clip = "off") +
    theme_tree() +
    theme(
        legend.position = "inside",
        legend.position.inside = c(1, 0.3),
        legend.background = element_rect(color = NA, fill = NA),
        legend.box.background = element_rect(color = "grey10", fill = "white"),
...
p2 <- tr_seq_core %>%
    ...
    # geom_label2(aes(subset=(node %in% c(31,37,39))), label = c("Ensifer spp.", "Ensifer meliloti", "Ensifer medicae"), label.size = 0, fill = NA, nudge_x = c(20, -5, 0) *1e-4, nudge_y = c(1, 1, -1), hjust = 1, fontface = "italic") +
    # geom_treescale(x = 0, y = 28, width = 0.001) +
    scale_color_manual(values = c(site_group_colors, `bootstrap>95%`="grey50"), name = NULL, breaks = c("suburban", "urban", "bootstrap>95%")) +
    scale_linetype_manual(values = c(1,5), name = NULL) +
    coord_cartesian(clip = "off") +
    theme_tree() +
    theme(
...
ggsave(here::here("forposter/trees.pdf"), p, width = 6, height = 8)
