...
get_distl_gra <- function (set_name) {
    tt <- read_gpas(set_name)
    gids <- names(tt$gpa[,-1])
    ...
    distl %>%
        ...
        filter(gradient1 == gra, gradient2 == gra) %>%
...
tree_wrapper <- function (set_name) {
    tr_gpa <- compute_gpa_tree(set_name)
    distl_gra <- get_distl_gra(set_name)
    tr_ani <- compute_ani_tree(distl_gra)
    tr_kmer <- compute_kmer_tree(distl_gra)
    write.tree(tr_gpa, paste0(folder_data, "phylogenomics_analysis/trees/", set_name, "/tr_gpa.tree"))
    write.tree(tr_ani, paste0(folder_data, "phylogenomics_analysis/trees/", set_name, "/tr_ani.tree"))
    write.tree(tr_kmer, paste0(folder_data, "phylogenomics_analysis/trees/", set_name, "/tr_kmer.tree"))
}

tree_wrapper("elev_med")
tree_wrapper("urbn_mel")
